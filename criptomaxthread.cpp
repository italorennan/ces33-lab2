...
void* mult(void* arg){
   ...
   //for (int i = 0; i < inputColumns; i++)
      for (int j = 0; j < SIZE; j++) 
         (thisData->C)[thisData->core][thisData->core2] += 
            (thisData->A)[thisData->core][j] * 
            (thisData->B)[j][thisData->core2];
}
...
void* opCode(void* arg){
   ...
   while (!finishOp)
   {
      ...
      else
      {
         ...
         pthread_t opThreads[SIZE][outputColumns];

         for (int i = 0; i < SIZE; i++){
            for (int j = 0; j < outputColumns; j++){ 
            td.core = i;
            td.core2 = j;
            pthread_create(&opThreads[i][j], NULL, 
               mult, (void *)&td);
            }
         }
         ...
         for (int i = 0; i < SIZE; i++)
            for (int j = 0; j < outputColumns; j++)
               pthread_join(opThreads[i][j], NULL);

         //matrixMultiplication(cod, input, 
         //output, SIZE, outputColumns, SIZE);
        ...
        // Codificar dado para output
            ...
            for (int i = 0; i < SIZE; i++) 
               for (int j = 0; j < outputColumns ; j++) {
                  td.core = i;
                  td.core2 = j;
                  pthread_create(&opThreads[i][j], NULL, mult, (void *)&td);
            }
            // Esperar threads terminarem execucao
            for (int i = 0; i < SIZE; i++)
               for (int j = 0; j < outputColumns; j++)
